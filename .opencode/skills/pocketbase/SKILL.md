...
- **OAuth2 auto-create is unreliable (0.40.4) — pre-create user records instead.** Auto-create for a new email fails with `400 Failed to authenticate` / `failed to save linked rel: collectionRef/provider/recordRef: Value must be unique` (even with all custom fields optional; indexes are fine, cause unresolved). The email-LINK path (pre-existing user record + Google login) works perfectly. Pattern: create the `users` record when the coach registers a client, then OAuth2 links by email on first sign-in.
